...
try:
    assert_equal(poco(text="不可與目前姓名相同").exists(), True, "確認\"姓名顯示不對\"選項輸入姓名相同時出現提示訊息")
...
try:
    assert_exists(Template(r"tpl1757580149919.png", record_pos=(-0.001, 0.882), resolution=(1080, 2400)), "確認\"姓名顯示不對\"選項裡，只輸入姓氏，\"送出審核\"按鈕反灰")
except Exception as e:
    log(e, snapshot=True)
...
try:
    assert_exists(Template(r"tpl1757580149919.png", record_pos=(-0.001, 0.882), resolution=(1080, 2400)), "確認\"姓名顯示不對\"選項，只輸入名字，\"送出審核\"按鈕反灰")
except Exception as e:
    log(e, snapshot=True)
...
try:
    assert_exists(Template(r"tpl1757580564993.png", record_pos=(0.0, 0.885), resolution=(1080, 2400)), "確認\"姓名顯示不對\"選項，姓名和名字皆輸入，\"送出審核\"按鈕亮起")
except Exception as e:
    log(e, snapshot=True)
    
poco(text="送出審核").click()
try:
    assert_equal(poco(text="請確認姓名與銀行帳戶一致").exists(), True, "確認\"姓名顯示不對\"選項，送出審核會跳出\"送請確認姓名與銀行帳戶一致\"的視窗")
except Exception as e:
    log(e, snapshot=True)

poco(text="返回修改").click()
try:
    assert_equal(poco(text="填寫正確姓名").exists(), True, "確認\"姓名顯示不對\"選項，送出審核後點擊\"返回修改\"按鈕會回到填寫正確姓名頁面")
except Exception as e:
    log(e, snapshot=True)